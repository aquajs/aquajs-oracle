...
AquaJsOracle.prototype.initConnPool = function () {
    if (this.pool === undefined || this.pool === null) {
        var aquaJsPool = oracle_pool.Pool({
            ...
            validateAsync  : function (connection,callback) {
                var cbFlag = true;

                $logger.debug("Validating the acquired  connection..");

                var timeout = setTimeout(function(callback) {
                    clearTimeout(timeout);
                    if(cbFlag) {
                        $logger.info("Connection Timed out while executing test Query....");
                        cbFlag = false ;
                        callback(false);
                    }
                }, queryExecTimeOut,callback);

                connection.execute("select 1 from dual", [], function (err) {
                    if (err) {
                        $logger.debug("Invalid Connection with Error" + err && jsonSafeStringify(err.message));
                        if(cbFlag) {
                            cbFlag = false ;
                            return callback(false);
                        }
                    } else {
                        $logger.debug("Connection is Valid and Ready to Use ... ");
                        if(cbFlag) {
                            cbFlag = false ;
                            callback(true);
                        }
